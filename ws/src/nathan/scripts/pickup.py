...
def setup_robotiq():
    ...
    return robotiq_client, robotiq_closed, robotiq_open
...
def setup_barrett():
    ...
    return barrett_open, barrett_close

def setup_scene():
    scene.clear()
    
    #Add the table to the scene (1500mm x 700mm x 20mm)
    add_box("table", [0.85, -0.33, 0.01], [0.70, 1.5, 0.02])
...
if __name__ == "__main__":
    left_pose_seq = {
        ...
        # Adjust based on place position
    ...
    if len(sys.argv) == 1 or sys.argv[1] not in ["--pick", "--place", "--assemble", "--manual"]:
        ...
        # Instantiate MoveGroupCommander for each arm
        left_arm_group, right_arm_group = setup_arms()

        print("MoveIt initialised")

        # Robotiq init
        print("Connected to Robotiq gripper...")
        robotiq_client, robotiq_closed, robotiq_open = setup_robotiq()
        print("Robotiq gripper ready")

        # Barret init
        print("Connecting to Barrett hand...")
        barrett_open, barrett_close = setup_barrett()
        print("Barrett hand ready")
