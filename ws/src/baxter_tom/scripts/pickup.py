...
if __name__ == "__main__":
    left_pose_seq = {
        ...
        "pre-pickup":   [-1.111, -0.725, +0.930, +0.846, -0.638, +1.686, +0.261],
    ...
    try:
        ...
        if (sys.argv[1] == "--pick"):
            ...
            input("Press enter to open grippers")
            barrett_init()
            ...
            input("Press enter to move right arm to grasp position")
            move_to_joints(right_arm_group, right_pose_seq["retreat"])

            input("Press enter to close barrett hand")
            barrett_close()
            attach_object("pipe", "bh_base_link")
            #"""
            input("Press enter to start assembly")
            print("--Moving to probing position")
            # Calls to Aidan's code
            moveTo(probe_pos)
            input("Press enter to probe surface height")
            resp = probeSurfaceClient()
            # from testing this should be ~0.21
            zHeight = resp.zHeight - 0.005
            print(f"--zHeight: {zHeight}")
            input("Press enter to start search sequence")
            ...
            while True:
                cmd = input("Enter arm to move (l/r) or 'q' to quit: ")
                if cmd.lower() == 'q':
                    break
                elif cmd.lower() == 'l':
                    joints = input("Enter 7 joint values separated by spaces or 'c' or 'o' to close/open the gripper: ").split(", ")
                    ...
                    joints = input("Enter 7 joint values separated by spaces or 'c' or 'o' to close/open the hand: ").split(", ")
                    if len(joints) == 1 and joints[0].lower() in ['c', 'o']:
                        if joints[0].lower() == 'c':
                            print("Closing hand")
                            barrett_close()
        ...
        input("Task complete. Press enter to exit")
        #"""
